...
<?php $__env->startSection('header_admin', 'Manajemen Galeri Kegiatan Pondok'); ?>
                                ...
                                <tr class="even:bg-gray-50 hover:bg-gray-100 transition-colors duration-150">
                                    <td class="py-4 px-6">
                                        <?php if($gallery->cover_image): ?>
                                            <img src="<?php echo e(asset('storage/' . $gallery->cover_image)); ?>" alt="<?php echo e($gallery->title); ?>" class="h-16 w-16 object-cover rounded-md border border-gray-200 shadow-sm">
                                        <?php else: ?>
                                            <span class="text-gray-400 text-sm italic">Tidak ada</span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="py-4 px-6 text-sm font-medium text-gray-900"><?php echo e($gallery->title); ?></td>
                                    <td class="py-4 px-6 text-sm text-gray-600"><?php echo e($gallery->images->count()); ?></td>
                                    <td class="py-4 px-6 text-sm">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full <?php echo e($gallery->is_published ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'); ?>">
                                            <?php echo e($gallery->is_published ? 'Publik' : 'Draft'); ?>
                                    ...
                                    <td class="py-4 px-6 text-center text-sm font-medium whitespace-nowrap">
                                        ...
                                        <?php endif; ?>
                                        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('edit galleries')): ?> 
                                            <a href="<?php echo e(route('admin.galleries.edit', $gallery)); ?>" class="inline-flex items-center text-sm px-4 py-2 bg-indigo-600 border border-transparent rounded-full font-semibold text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:ring-offset-2 transition ease-in-out duration-150 shadow-md ml-3">
                                        ...
                                        <?php endif; ?>
                                        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('delete galleries')): ?> 
                                            <form action="<?php echo e(route('admin.galleries.destroy', $gallery)); ?>" method="POST" class="inline-block ml-3" onsubmit="return confirm('Apakah Anda yakin ingin menghapus album ini beserta semua gambarnya? Aksi ini tidak dapat dibatalkan.');">
                                                <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/husnulfuadifebriansyah/Documents/dari git/ponpesdibama/resources/views/admin/galleries/index.blade.php ENDPATH**/ ?>