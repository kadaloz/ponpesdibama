...
@section('header_admin', 'Manajemen Program Pendidikan')
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($allPrograms as $program)
                                <tr class="even:bg-gray-50 hover:bg-gray-100 transition-colors duration-150">
                                    <td class="py-4 px-6 text-sm font-medium text-gray-900">{{ $program->name }}</td>
                                    <td class="py-4 px-6 text-sm text-gray-600">{{ $program->duration ?? '-' }}</td>
                                    ...
                                    <td class="py-4 px-6 text-center text-sm font-medium whitespace-nowrap">
                                        ...
                                        <form action="{{ route('admin.programs.destroy', $program) }}" method="POST" class="inline-block ml-3" onsubmit="return confirm('Apakah Anda yakin ingin menghapus program ini? Aksi ini tidak dapat dibatalkan.');">
                                            @csrf
                                            @method('DELETE')
