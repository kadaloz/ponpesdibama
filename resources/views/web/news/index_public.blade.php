...
@section('main_content')
    <!-- Hero Section -->
    <section class="py-20 md:py-28 bg-teal-700 text-white rounded-b-3xl shadow-xl">
        <div class="container mx-auto px-4 text-center">
            ...
            <a href="{{ url('/') }}" class="mt-10 inline-flex items-center px-8 py-3 bg-white text-teal-700 text-base font-semibold rounded-full shadow-md hover:bg-teal-100 transition transform hover:scale-105">
    ...
    <section class="bg-white border-b border-gray-100 shadow-sm py-6">
        <div class="container mx-auto px-4">
            <form method="GET" action="{{ route('news.index_public') }}" class="flex flex-col md:flex-row items-center justify-between gap-4">
                <div class="flex items-center gap-2 w-full md:w-auto">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari berita..." class="w-full md:w-64 px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach ($allNews as $newsItem)
                        ...
                        <div class="bg-white rounded-xl shadow-lg overflow-hidden transform transition-transform duration-300 hover:scale-105 hover:shadow-xl border border-gray-200">
                            ...
                            @endif
                            <div class="p-6">
                                <span class="text-sm text-gray-500 block mb-2">{{ $newsItem->published_at ? \Carbon\Carbon::parse($newsItem->published_at)->format('d F Y') : 'Tanggal Tidak Tersedia' }}</span>
                                <h3 class="text-xl font-bold text-teal-700 my-2">{{ $newsItem->title }}</h3>
                                <p class="text-gray-700 text-base mb-4">
                                    {{ Str::limit(strip_tags($newsItem->content), 100) }}
                                </p>
                                <a href="{{ route('news.show', $newsItem->slug) }}" class="inline-flex items-center text-teal-600 hover:text-teal-800 font-semibold group transition-colors duration-200">
                                    Baca Selengkapnya
