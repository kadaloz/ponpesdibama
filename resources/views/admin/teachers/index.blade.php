...
@section('header_admin', 'Manajemen Data Pengajar')
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-8 text-gray-900">
            <div class="flex flex-col md:flex-row justify-between items-center mb-8">
                ...
                @can('create teachers') {{-- Tombol Tambah hanya jika memiliki izin create teachers --}}
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($allTeachers as $teacher)
                                <tr class="even:bg-gray-50 hover:bg-gray-100 transition-colors duration-150">
                                    <td class="py-4 px-6 text-sm font-medium text-gray-900">{{ $teacher->nip ?? '-' }}</td>
                                    <td class="py-4 px-6 text-sm text-gray-900">{{ $teacher->full_name }}</td>
                                    <td class="py-4 px-6 text-sm text-gray-600">{{ $teacher->specialization ?? '-' }}</td>
                                    ...
                                    <td class="py-4 px-6 text-center text-sm font-medium whitespace-nowrap">
                                        ...
                                        @endcan
                                        @can('delete teachers') {{-- Tombol Hapus --}}
                                            <form action="{{ route('admin.teachers.destroy', $teacher->id) }}" method="POST" class="inline-block ml-3" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data pengajar ini? Aksi ini tidak dapat dibatalkan.');">
                                                @csrf
                                                @method('DELETE')
