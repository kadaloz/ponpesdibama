...
@section('header_admin', 'Edit Pengguna')
        ...
        <div class="p-6 text-gray-900">
            <form action="{{ route('admin.users.update', $user) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-teal-500 focus:border-teal-500" value="{{ old('name', $user->name) }}" required>
                    @error('name')
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-teal-500 focus:border-teal-500" value="{{ old('email', $user->email) }}" required>
                    @error('email')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                            ...
                            @foreach ($roles as $id => $name)
                                <div class="flex items-center">
                                    <input type="checkbox" name="roles[]" id="role_{{ $id }}" value="{{ $id }}"
                                           ...
                                           {{ ($user->id == auth()->id() && $name == 'admin' && count($userRoles) == 1) ? 'disabled' : '' }}> {{-- Admin tidak bisa menghapus peran adminnya sendiri jika itu satu-satunya peran --}}
                                    <label for="role_{{ $id }}" class="ml-2 block text-sm text-gray-900">
                                        {{ ucfirst(str_replace('_', ' ', $name)) }}
                                        @if ($name == 'admin' && $user->id == auth()->id() && count($userRoles) == 1)
